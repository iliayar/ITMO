...
#ifdef LOG_DEBUG
#define DBG(name) std::cerr << "DBG[" #name "]: "
#else
#define DBG(name) constexpr if (false) std::cerr
#endif
...
void assert(bool val, const char *msg = "assertion failed") {
  if (!val) {
    std::cerr << msg;
    exit(1);
  }
...
public:
  ...
  auto begin() -> std::vector<T>::iterator { return data_.begin(); }
  auto begin() const -> std::vector<T>::const_iterator { return data_.begin(); }

  auto end() -> std::vector<T>::iterator { return data_.end(); }
  auto end() const -> std::vector<T>::const_iterator { return data_.end(); }
...
auto projection(vF2 const &v, Idxs const &idxs) -> vF2 {
  ...
  // DBG(projection) << PARAM(v) << PARAM(idxs) << PARAM(res) << std::endl;
...
auto make_trellis(mF2 mat) -> TF2 {
  ...
  // DBG(make_trellis 0) << PARAM(n) << PARAM(k) << std::endl << mat <<
  // std::endl;
  ...
  // DBG(make_trellis 1) << PARAM(spans) << PARAM(active_rows) << std::endl;
  ...
  for (int i = 0; i < k; ++i) {
    ...
    // DBG(make_trellis->while 0)
    //     << PARAM(i) << PARAM(te) << PARAM(te.size()) << std::endl;

    auto c = vF2::zero(n);
    auto ce = c;
    increment(c);
    // DBG(make_trellis->while 1) << PARAM(c) << PARAM(ce) << std::endl;
    while (c != ce) {
      // DBG(make_trellis->while->while 0) << PARAM(c) << std::endl;
      auto t = projection(c, te);
      auto vp = extend_zeros(t, n, active_rows[i]);
      auto vi = extend_zeros(t, n, active_rows[i + 1]);
      res.back()[vp].emplace(
          vi, EF2{
                  .v = sum(projection(mat.get(all, i).vec(), te) * t),
              });
      increment(c);
    }
...
void handle_commands(Encoder const &encoder, Decoder const &decoder) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    if (cmd == "Encode") {
      vector<typename Encoder::ValueType> v =
          vector<typename Encoder::ValueType>::zero(encoder.length());
      ss >> v;
...
int main() {
  ...
  SoftTrellisDecoder<F2> decoder;
  handle_commands(encoder, decoder);
}
