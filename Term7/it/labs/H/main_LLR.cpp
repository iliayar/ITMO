...
struct ListDecoder {
  ...
  auto getArrayPointer_LLR(int la, int l) const -> std::vector<double> & {
    ...
    if (arrayReferenceCount[la][s] == 1) {
      ...
      arrayPointer_LLR[la][sp].assign(arrayPointer_LLR[la][s].begin(),
                                      arrayPointer_LLR[la][s].end());
  ...
  void recursivelyCalcLLR(int la, int phi) const {
    if (la == 0)
      return;
    ...
    for (int l = 0; l < L_; ++l) {
      ...
      auto &LLR_la = getArrayPointer_LLR(la, l);
      auto &LLR_la1 = getArrayPointer_LLR(la - 1, l);
      auto &C_la = getArrayPointer_C(la, l);

      for (int b = 0; b < (1 << (m_ - la)); ++b) {
        if (phi % 2 == 0) {
          if (40 >
              std::max(std::abs(LLR_la1[b]), std::abs(LLR_la1[SZ(la) + b]))) {
            LLR_la[b] = std::log(
                (std::exp(LLR_la1[b] + LLR_la1[SZ(la) + b]) + 1) /
                (std::exp(LLR_la1[b]) + std::exp(LLR_la1[SZ(la) + b])));
          } else {
            LLR_la[b] =
                sgn(LLR_la1[b]) * sgn(LLR_la1[SZ(la) + b]) *
                std::min(std::abs(LLR_la1[b]), std::abs(LLR_la1[SZ(la) + b]));
          }
        } else {
          LLR_la[b] = (1. - 2. * C_la[b][0]) * LLR_la1[b] + LLR_la1[SZ(la) + b];
        }
      }
    }
  }
  ...
  void continuePaths_FrozenBit(int phi) const {
    for (int l = 0; l < L_; ++l) {
      ...
      auto &LLR_p = getArrayPointer_LLR(m_, l);
      pathMetric[l] += std::log(1 + std::exp(-LLR_p[0]));
    }
  }
  ...
  void continuePaths_UnfrozenBit(int phi) const {
    ...
    for (int l = 0; l < L_; ++l) {
      if (pathIndexInactive(l)) {
        ...
        auto &LLR_m = getArrayPointer_LLR(m_, l);
        probForks[l][0] = -(pathMetric[l] + std::log(1 + std::exp(-LLR_m[0])));
        probForks[l][1] = -(pathMetric[l] + std::log(1 + std::exp(LLR_m[0])));
    ...
    for (int l = 0; l < L_; ++l) {
      ...
      if (contForks[l][0] && contForks[l][1]) {
        ...
        pathMetric[l] += std::log(1 + std::exp(-LLR_m[0]));
        auto &LLR_m = getArrayPointer_LLR(m_, lp);
        pathMetric[lp] += std::log(1 + std::exp(LLR_m[0]));
      } else {
        if (contForks[l][0]) {
          C_m[0][phi % 2] = 0;
          pathMetric[l] += std::log(1 + std::exp(-LLR_m[0]));
  ...
  auto decode(vec<double> y, std::optional<double> param) const -> VF2 {
    ...
    auto &LLR0 = getArrayPointer_LLR(0, l);

    for (int b = 0; b < n_; ++b) {
      double snr =
          std::sqrt((n_ / k_) * std::pow(10., -param.value_or(1.) / 10.));
      LLR0[b] = y.get(b) / snr;
    }

    for (int phi = 0; phi < n_; ++phi) {
      recursivelyCalcLLR(m_, phi);
      if (is_freeze(F_, phi)) {
  ...
  auto init() {
    ...
    arrayPointer_LLR.resize(m_ + 1, std::vector<std::vector<double>>(L_));
    ...
    for (int la = 0; la <= m_; ++la) {
      for (int s = 0; s < L_; ++s) {
        arrayPointer_LLR[la][s] = std::vector<double>(1 << (m_ - la));
  ...
  mutable std::vector<std::vector<std::vector<double>>> arrayPointer_LLR;
  ...
  mutable std::vector<double> pathMetric;
