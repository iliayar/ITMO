...
Рассмотрим \(S\) из [[параметризация_8][определения]]: \todo
- \((*, *)\), получим \(\lambda_\to\) \\
  /Характерные представители/: \(\lambda x. x\)
- \((*, *), \(\square, *)\), получим \(\lambda_2, F\) \\
  /Характерные представители/: \(\Lambda \alpha. \lambda x: \alpha. x\)
  #+begin_remark org
  \(\Lambda \alpha. b \approx \lambda \alpha: *. b\)
  #+end_remark
- \((*, *), (\square, *), (\square, \square)\) это \(\lambda \omega\)
- \((\square, \square), (*, *)\) это \(\lambda \omega\) слабая \\
  /Характерные представители/: =int[]=
