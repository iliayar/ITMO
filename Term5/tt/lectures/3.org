...
#+end_theorem
#+begin_remark org
Выше было выписан только импликационный фрагмент ИИВ: \(I_\to, E_\to, Ax\)
#+end_remark
#+begin_symb org
\(\Gamma \vdash_\to \tau\)
#+end_symb
#+begin_theorem org
Импликациооныйй фрагмент ИИВ (И. Ф. ИИВ) замкнут относительно
доказуемости. Если \(\Gamma \vdash \tau\) и \(\tau\) содержит только \(\alpha, \to\), то
\(\Gamma \vdash_\to \tau\). И если \(\Gamma \vdash_\to \tau\), то \(\Gamma \vdash \tau\)
#+end_theorem
#+begin_proof org
