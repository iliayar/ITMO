...
pub mod gramma {
	...
	pub fn rule(&mut self, nonterm: String, right: Vec<String>) -> &mut Self {
	    let mut rule = self.make_rule(nonterm.clone(), right);
	...
	pub fn add_rules(&mut self, nt: String, rights: Vec<Vec<String>>) -> &mut Self {
	    for right in rights.into_iter() {
		self.rule(nt.clone(), right);
	    }
	    return self;
	}
