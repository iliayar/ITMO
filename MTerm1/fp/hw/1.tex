...
\begin{document}
    ...
    \begin{proof}
        ...
        \((\lambda x^0\ldotp y~y~(\lambda y^1\ldotp x^0~((\lambda z^2\ldotp z^2~y^1~x^0)~(\lambda x^3\ldotp z~x^3))))\)
    \end{proof}
    ...
    \begin{proof}
        ...
        \(\textbf{pow} \coloneqq \lambda a~b\ldotp b~(\term{mult}~a)~\term{1}\)

        Примеры:
        \begin{gather}
            \term{pow}~\term{3}~\term{2} \betasteps \term{2}~(\term{mult}~\term{3})~\term{1} \betasteps \\
            \betasteps (\term{mult}~\term{3})~((\term{mult}~\term{3})~\term{1}) \betasteps \\
            \betasteps (\term{mult}~\term{3})~\term{3} \betasteps \term{9}
        \end{gather}

        \begin{gather}
            \term{pow}~\term{2}~\term{0} \betasteps \term{0}~(\term{mult}~\term{2})~\term{1} \betasteps \term{1}
        \end{gather}

        \begin{gather}
            \term{pow}~\term{1}~\term{3} \betasteps \term{3}~(\term{mult}~\term{1})~\term{1} \betasteps \\
            \betasteps (\term{mult}~\term{1})~((\term{mult}~\term{1})~((\term{mult}~\term{1})~\term{1})) \betasteps \\
            \betasteps (\term{mult}~\term{1})~((\term{mult}~\term{1})~\term{1}) \betasteps \term{1}
        \end{gather}
    \end{proof}
