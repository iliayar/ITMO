...
\end{minted}
    ...
    \begin{proof}
        \(\term{pred}\) и \(\term{minus}\) из предыдущего ДЗ.
        \begin{gather}
    ...
    \begin{proof}
        \begin{gather}
            \term{fib}~n \coloneqq \term{if}~(\term{equal}~n~0)~1~(\term{if}~(\term{equal}~n~1)~1~(\term{add}~(\term{fib}~(\term{minus}~n~1))~(\term{fib}~(\term{minus}~n~2))))
        \end{gather}
        Проверка:
        \begin{align}
            \term{fib}~3 & \eqbeta \term{if}~(\term{equal}~3~0)~1~(\term{if}~(\term{equal}~3~1)~1~(\term{add}~(\term{fib}~(\term{minus}~3~1))~(\term{fib}~(\term{minus}~3~2)))) \eqbeta \\
                         & \eqbeta \term{add}~(\term{fib}~2)~(\term{fib}~1) \eqbeta \term{add}~(\term{add}~(\term{fib}~1)~(\term{fib}~0))~1 \eqbeta \\
                         & \eqbeta \term{add}~(\term{add}~1~1)~1 \eqbeta 3
        \end{align}
    \end{proof}
    ...
    \begin{proof}
        \begin{gather}
            \term{sum}~xs \coloneqq \term{fold}~xs~\term{add}~0
        \end{gather}
        Проверка:
        \begin{align}
            \term{sum} [1, 2, 3] \eqbeta \term{add}~1~(\term{add}~2~(\term{add}~3~0)) \eqbeta
        \end{align}
    \end{proof}
    ...
    \begin{proof}
        \begin{gather}
            \term{length}~xs \coloneqq \term{fold}~xs~(\lambda x~l\ldotp \term{add}~1~l)~0
        \end{gather}
        Проверка:
        \begin{align}
            \term{length}~[3, 2, 1] & \eqbeta (\lambda i~l\ldotp \term{add}~1~l)~3~(\term{fold}~[2, 1]~(\lambda i~l\ldotp \term{add}~1~l)~0)) \eqbeta \\
                                    & \eqbeta \term{add}~1~(\term{fold}~[2, 1]~(\lambda i~l\ldotp \term{add}~1~l)~0)) \eqbeta \\
                                    & \eqbeta \term{add}~1~(\term{add}~1~(\term{add}~1~0)) \eqbeta 3
        \end{align}
    \end{proof}
