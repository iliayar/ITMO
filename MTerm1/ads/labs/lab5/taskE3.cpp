...
// Generated at 2025-06-14 22:49:06.933414 
// By iliayar
...
template <typename I>
auto kth(I begin, I end, int k) {
    int len = end - begin + 1;
    DBG() << "kth " << vector(begin, end) << " " << len << " " << k << endl;
    if (len < 1) exit(1);
    if (len == 1) {
        if (k != 0) exit(1);
        return *begin;
