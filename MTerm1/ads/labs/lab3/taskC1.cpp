...
// Generated at 2024-09-28 23:30:41.852331 
// By iliayar
...
//##################CODE BEGIN#############
template <typename F>
std::pair<int, int> bs(int l, int r, F pred) {
  while (r - l > 1) {
    int m = (l + r) / 2;
    if (pred(m)) {
      l = m;
...
void sol() {
    ...
    for (int i = 0; i < k; ++i) {
        int L, R; cin >> L >> R;
        auto [Ll, Lr] = bs(0, a.size(), [&](int m) { return a[m] < L; });
        auto [Rl, Rr] = bs(0, a.size(), [&](int m) { return a[m] <= R; });
        cout << Rr - Ll - 1 << endl;
    }
}
//##################CODE END###############
