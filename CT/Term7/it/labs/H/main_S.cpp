...
void handle_commands(Encoder const &encoder, Decoder &decoder,
...
struct ListDecoder {
  ...
  auto getArrayPointer_S(int la, int l) -> std::vector<double> & {
    ...
    if (arrayReferenceCount[la][s] == 1) {
      ...
      arrayPointer_S[la][sp].assign(arrayPointer_S[la][s].begin(),
                                    arrayPointer_S[la][s].end());
    ...
    return arrayPointer_S[la][sp];
  }

  auto getArrayPointer_C(int la, int l) -> std::vector<std::array<int, 2>> & {
    auto s = pathIndexToArrayIndex[la][l];
    int sp;
  ...
  void recursivelyCalcS(int la, int phi) {
    if (la == 0) {
      return;
    }
    ...
    for (int l = 0; l < L_; ++l) {
      ...
      auto &S_la = getArrayPointer_S(la, l);
      auto &S_la1 = getArrayPointer_S(la - 1, l);
      auto &C_la = getArrayPointer_C(la, l);

      for (int b = 0; b < SZ(la); ++b) {
        if (phi % 2 == 0) {
          S_la[b] = sgn(S_la1[b]) * sgn(S_la1[SZ(la) + b]) *
                    std::min(std::abs(S_la1[b]),
                             std::abs(S_la1[SZ(la) + b]));
        } else {
          int u = C_la[b][0];
          S_la[b] = (1. - 2. * u) * S_la1[b] + S_la1[SZ(la) + b];
        }
      }

      DBG_OPT("recursivelyCalcS", DBG_BASIC)
          << PARAM(phi) << PARAM(la) << PARAM(l) << PARAM(S_la1) << PARAM(S_la)
          << std::endl;
      DBG_OPT("recursivelyCalcS PRECISE", DBG_BASIC)
          << PARAM(getCW(l, la, 0)) << PARAM(getCW(l, la, 0)) << std::endl;
    }
  }

  double calcPathMetric(double S, int b) {
    // sgn(S) == (-1)^b
    if ((sgn(S) < 0 && b == 1) || (sgn(S) > 0 && b == 0))
      return 0;
    return -std::abs(S);
  }
  ...
  void continuePaths_FrozenBit(int phi) {
    for (int l = 0; l < L_; ++l) {
      ...
      auto &S_p = getArrayPointer_S(m_, l);
      pathMetric[l] += calcPathMetric(S_p[0], 0);

      DBG_OPT("continuePaths_FrozenBit TRANS", DBG_TRANSITION || DBG_BASIC)
          << PARAM(l) << PARAM(0) << std::endl;
    }
  }
  ...
  void continuePaths_UnfrozenBit(int phi) {
    ...
    for (int l = 0; l < L_; ++l) {
      if (pathIndexInactive(l)) {
        ...
        auto &S_m = getArrayPointer_S(m_, l);
        probForks[l][0] = pathMetric[l] + calcPathMetric(S_m[0], 0);
        probForks[l][1] = pathMetric[l] + calcPathMetric(S_m[0], 1);
    ...
    for (int l = 0; l < L_; ++l) {
      ...
      if (contForks[l][0] && contForks[l][1]) {
        ...
        DBG_OPT("continuePaths_UnfrozenBit TRANS", DBG_TRANSITION)
            << PARAM(l) << PARAM(0) << std::endl;
        DBG_OPT("continuePaths_UnfrozenBit TRANS", DBG_TRANSITION)
            << PARAM(lp) << PARAM(1) << std::endl;

        DBG_OPT("continuePaths_UnfrozenBit BEFORE", false)
            << PARAM(S_m[0]) << PARAM(pathMetric[l]) << PARAM(pathMetric[lp])
            << std::endl;
        pathMetric[l] += calcPathMetric(S_m[0], 0);
        auto &S_m = getArrayPointer_S(m_, lp);
        pathMetric[lp] += calcPathMetric(S_m[0], 1);
        DBG_OPT("continuePaths_UnfrozenBit BEFORE", false)
            << PARAM(pathMetric[l]) << PARAM(pathMetric[lp]) << std::endl;
      } else {
        if (contForks[l][0]) {
          C_m[0][phi % 2] = 0;
          pathMetric[l] += calcPathMetric(S_m[0], 0);
          ...
          C_m[0][phi % 2] = 1;
          pathMetric[l] += calcPathMetric(S_m[0], 1);
          DBG_OPT("continuePaths_UnfrozenBit TRANS", DBG_TRANSITION)
              << PARAM(l) << PARAM(1) << std::endl;
        }
      }
    }
  }
  ...
  void killPath(int l) {
    DBG_OPT("killPath", false) << PARAM(l) << std::endl;
    activePath[l] = false;
  ...
  auto clonePath(int l) -> int {
    auto lp = inactivePathIndices.back();
    inactivePathIndices.pop_back();

    DBG_OPT("clonePath", DBG_BASIC) << PARAM(l) << PARAM(lp) << std::endl;
  ...
  auto pathIndexInactive(int l) -> bool { return !activePath[l]; }
  ...
  void recursivelyUpdateC(int la, int phi) {
    ...
    for (int l = 0; l < L_; ++l) {
      ...
      DBG_OPT("recursivelyUpdateC PRECISE", false)
          << PARAM(psi) << PARAM(la) << PARAM(l) << PARAM(getCW(l, la, 0))
          << PARAM(getCW(l, la, 1)) << PARAM(getCW(l, la - 1, psi % 2))
          << std::endl;
    }
  ...
  auto findMostProbablePath() -> int {
    ...
    for (int l = 0; l < L_; ++l) {
      ...
      DBG_OPT("findMostProbablePath", DBG_BASIC)
          << PARAM(l) << PARAM(pathMetric[l]) << PARAM(getCW(l))
          << PARAM(getArrayPointer_S(m_, l)) << std::endl;
  ...
  auto getCW(int l, int la = 0, int s = 0) -> VF2 {
  ...
  void printCTree(int l) {
    for (int la = 0; la <= m_; ++la) {
      DBG("printCTree") << PARAM(getCW(l, m_ - la, 0))
                        << PARAM(getCW(l, m_ - la, 1)) << std::endl;
    }
  }

  auto decode(vec<double> y, std::optional<double> param) -> VF2 {
    initializeDataStructures();
    auto l = assignInitialPath();
    auto &S0 = getArrayPointer_S(0, l);

    for (int b = 0; b < n_; ++b) {
      S0[b] = y.get(b);
    }
    DBG_OPT("decode INIT", false) << PARAM(S0) << std::endl;

    for (int phi = 0; phi < n_; ++phi) {
      recursivelyCalcS(m_, phi);
      if (is_freeze(F_, phi)) {
      ...
      if constexpr (DBG_BASIC) {
        for (int l = 0; l < L_; ++l) {
          if (pathIndexInactive(l))
            continue;
          DBG("decode PATH")
              << PARAM(phi) << PARAM(l) << PARAM(pathMetric[l]) << std::endl;
        }
      }
    }
  ...
  auto init() {
    ...
    arrayPointer_S.resize(m_ + 1, std::vector<std::vector<double>>(L_));
    ...
    pathMetric.resize(L_);

    for (int la = 0; la <= m_; ++la) {
      for (int s = 0; s < L_; ++s) {
        arrayPointer_S[la][s] = std::vector<double>(SZ(la), 0.);
        arrayPointer_C[la][s] = std::vector<std::array<int, 2>>(SZ(la));
      }
  ...
  std::vector<std::vector<std::vector<double>>> arrayPointer_S;
  std::vector<std::vector<std::vector<std::array<int, 2>>>> arrayPointer_C;
  ...
  std::vector<std::array<double, 2>> probForks;
  std::vector<std::array<bool, 2>> contForks;
  std::vector<std::tuple<int, int>> probsRating;
};
