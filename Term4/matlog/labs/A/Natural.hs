...
showTree n = unlines $ reverse $ toList 0 n
    where
        ...
        toList d (st, AndDelLeft n) = showNode d st [n] "El&"
        toList d (st, AndDelRight n) = showNode d st [n] "Er&"
...
makeAndDelLeft psi maker s@(hypos, phi) = if phi `elem` hypos then (s, Axiom) else (s, AndDelLeft $ maker (hypos, And phi psi))

makeAndDelRight :: Expression -> (Statement -> Node) -> Statement -> Node
makeAndDelRight phi maker s@(hypos, psi) = if psi `elem` hypos then (s, Axiom) else (s, AndDelRight $ maker (hypos, And phi psi))
