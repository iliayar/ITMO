...
- \(D \subset \R\)
- \(h_0\) --- предлеьная точка в \(\overline{\R}\)
Семейство функций \(\{K_h\}_{h \in D}\) --- называется аппроксимативной единицей
- *АЕ1* :: \(\forall h \in D\), \(K_n \in L^1[-\pi, \pi]\), \(\int_{-\pi}^\pi K_h = 1\)
- *АЕ2* :: \(L_1\) --- нормы функций \(K_h\) ограничены в совокупности
   \[ \exists M\ \forall h\ \int_{[-\pi, \pi]} |K_h| \le M \]
- *АЕ3* :: \(\forall \delta \in (0, \pi) \quad \int_{E\delta} |K_h|\,dx \xrightarrow[h \to h_0]{} 0\)
#+end_definition
#+begin_remark org
\(K_h \ge 0\ \forall h\) \\
...
- *АЕ3'* \(K_h \in L^\infty[-\pi, \pi]\) и \(\forall \delta \in (0; \pi)\)
  \[ \esssup_{t \in E_\deta} |K_h(t)| \xrightarrow[h \to h_0]{} 0 \]
#+end_remark
#+begin_lemma org
...
2. \(g \in L^1[-\pi, \pi] \Rightarrow \Vert f*K_h - f \Vert_1 \xrightarrow[h \to h_0]{} 0\)
...
\[ |\delta_n - S| - \left|\frac{1}{n + 1}\sum_{k = 0}^n (S_k - S)\right| \le \frac{1}{n + 1}\sum |S_k - S| = \]
\[ = \frac{\sum_{k = 0}^{N_1}|S_k - S|}{n + 1} + \frac{\sum_{k = N_1 + 1}^n |S_k - S|}{n + 1} \]
\(\forall N\ \forall n > N\) эта дробь \(< \frac{\varepsilon}{2}\)
#+end_proof
#+begin_definition org
\(f \in L^1[-\pi, \pi]\), \(S_n(f)\) --- частичная сумма ряда Фурье
\[ G_n(f) = \frac{1}{n + 1}\sum_{k = 1}^n S_k(f) \] --- суммы Фейера
#+end_definition
#+begin_remark org
\[ S_n(f) = \int_{-\pi}^\pi f(x + t) D_n(t)\,dt \]
\[ G_n(f, x) = \int_{-\pi}^\pi f(x + t) \Phi_n(t)\,dt \]
, где \(\displaystyle\Phi_n = \frac{1}{n + 1}(D_0 + \dots + D_n) = \frac{1}{2\pi(n + 1)}\cdot \frac{\sin^2\left(\frac{n + 1}{2}t\right)}{\sin^2\frac{t}{2}}\)
#+end_remark
...
\[ \exists \lin_{n \to \infty} S_n(f, x)\text{ --- конечный}\]
, то \(S_n(f, x) \to f(x)\)
#+end_corollary
#+begin_proof org
\[ \sigma_n(f, x) \to f(x) \]
...
3. \[ \int_{-\pi}^\pi fg = \pi \left(\frac{a_0(f)a_0(h)}{2} + \sum_{k = 1}^{+\infty}a_k(f)a_k(g) + b_k(f)b_k(g)\rigth) \]
