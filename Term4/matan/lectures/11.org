...
1. \(f\) --- равномерно непрерывна. \\
   _Тогда_ \(\Vert f_n - f \Vert_\infty \xrightarrow[h \to 0]{} 0\), \\
   т.е. \(\sup_x |f(x + h) - f(x)| \to 0\)
2. <<nepr_11_2>>\(f \in L^p(\R^m),\ 1 \le p < +\infty\) \\
   _Тогда_ \(\Vert f_h - f_p \Vert \xrightarrow[h \to 0]{} 0\)
3. \(f \in \tilde{C}[0, T]\) \\
...
1. Для \(L^\infty\) непрерывности сдвига нет
   \[ f = \chi_{[0, 1]} \quad f_h = \chi_{[-h, 1 - h]} \]
   \[ \esssup |f - f_n| = 1\]
2. Во всех упомянутых [[nepr_11_2]] и [[nepr_11_4]] \\
   \[ h \mapsto \Ver f_h - f \Vert_p \]
...
- Скалярное произведение \(X \times X \to \R(\C)\)
  ...
  3. \(\pair{x, y} = \pair{y, x}\) (\(\pari{x, y} = \overline{\pair{y, x}}\))
  Неравенство Коши-Буняковского: \(|\pair{x, y}|^2 \le \pair{x, x}\pair{y, y}\)
  \[ \Vert x \Vert \xlongequals{\text{def}} \sqrt{\pair{x, x}} \]
...
#+begin_theorem org
\(\{e_k\}\) --- О.С. в \(\H\), \(x \in \H\). \(x = \sum_{k = 1}^\infty c_k e_k\), где \(c_k \in \R(\C)\) \\
_Тогда_
1. \(\{e_k\}\) --- Л.Н.З
...
1. \(\sum_{k = 1}^N \alpha_k e_k = 0\) \\
   \(\alpha_n \norm{e_k}^2 = 0\)
2. \[\pair{x, e_k} = 2 \sum\pair{c_je_j, e_k} \]
\todo
#+end_proof
...
--- называется коэффициентами Фурье элемента \(x\) по системе \(\{e_k\}\)
