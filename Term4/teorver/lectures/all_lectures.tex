...
\chapter{}
\label{sec:org172f87a}
\section{Статистическая вероятность}
\label{sec:orgc49ca8e}
...
\end{solution}
\chapter{}
\label{sec:orgbb3e38f}
\section{Условная вероятность}
\label{sec:org171c771}
\begin{symb}
...
\end{solution}
\chapter{}
\label{sec:orgd9d20cf}
\section{Схема Бернулли}
\label{sec:org4a9f23d}
\begin{definition}
...
\end{remark}
\begin{examp}
\hyperref[orgf84353b]{Бросаем кость}
...
\end{remark}
\begin{proof}
По \hyperref[org8911974]{свойству} \(\text{Cov}(\xi, \eta) = 0\)
\end{proof}
\begin{remark}
...
\end{proof}
\begin{property}
Если \(F(x)\) непрерывна в точке \(x_0\), то \(p(\xi = 0) = 0\). Следствие из \ref{org7c3276d}
\end{property}
\begin{property}
...
\end{property}
\begin{proof}
Из определения или из \ref{org6f92433}
\end{proof}
\begin{property}
\(p(\xi = x_0) = 0\)
\end{property}
...
\end{property}
\begin{property}
Если для \(f(x)\) выолнено свойства \ref{org2404c0a} и \ref{org968858c} то она является плотностью некоторой случайной величины
\end{property}
\subsection{Числовые характеричтики}
\label{sec:org731910e}
\begin{definition}
...
\end{property}
\chapter{10 апреля}
\label{sec:orgc0fa5d3}
\section{Стандартное абсолюютно непрерывное распределение}
\label{sec:org7b5cb38}
\subsection{Равномерное распределение}
\label{sec:orgccedb6d}
\begin{definition}
Случайная величина \(\xi\) \textbf{равномерно} распределена на \([a, b]\) если ее плотность постоянна на этом отрезке
\[ f(x) = \begin{cases}
0 & x < a \\
