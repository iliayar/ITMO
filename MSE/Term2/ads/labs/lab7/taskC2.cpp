...
// Generated at 2026-03-25 16:41:03.333275 
// By iliayar
// touch: 1
...
void sol() {
    ...
    FUNC(int, get_hash, int l1, int r1) {
        int xp = powmod(B, r1 - l1);
        return (h[r1] - ((h[l1] * xp) % M) + M) % M;
    };

    int hs = get_hash(0, s.length());
    for (int i = 1; i <= s.length(); ++i) {
        int p = s.length() % i;
        if (p != 0 && get_hash(0, p) != get_hash(s.length() - p, s.length())) {
            continue;
        }
