...
// Generated at 2026-03-08 20:43:38.690068 
// By iliayar
...
void dbg_print(ostream& out, Node* node, int depth = 0) {
    ...
    out << string(depth*2, ' ') << "y=" << node->y << " size=" << node->size << endl;
...
void collect(vint& out, Node* node) {
    if (!node) return;
    propagate_node(node);
    collect(out, node->left);
    out.push_back(node->value);
    collect(out, node->right);
}
...
void sol() {
    ...
    for (int q = 0; q < m; ++q) {
        ...
        if (cmd == "+") {
            int i, j; cin >> i >> j; i--; j--;
            join(get_root(a[i]), get_root(a[j]));
        } else if (cmd == "!") {
            int i, k; cin >> i >> k; i--;
            rotate(get_root(a[i]), k);
        } else if (cmd == "-") {
            int i, k; cin >> i >> k; i--;
            split(get_root(a[i]), k);
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        auto root = get_root(a[i]);
        if (b.contains(root)) {
            continue;
        }
        b[root] = vs.size();
        vs.emplace_back();
        collect(vs.back(), root);
    }
    ...
    // cout << b.size() << endl;
    // unordered_set<Node*> was;
    // for (int i = 0; i < n; ++i) {
    //     auto root = get_root(a[i]);
    //     if (was.contains(root)) {
    //         continue;
    //     }
    //     was.insert(root);
    //     cout << b[root] << " ";
