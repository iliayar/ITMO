...
\begin{document}
    ...
    \begin{proof}
        \begin{gather}
            x^{?_0}\ldotp \underbrace{\underbrace{\underbrace{x^{?_0} \ap \term{K}^{?_1 \to ?_2 \to ?_1}}_{?_4} \ap \term{I}^{?_3 \to ?_3}}_{?_5} \ap x^{?_0}}_{?_6} \\
        ...
        \begin{gather}
            ...
            \tau = ?_0 \to ?_6
        \end{gather}
        \(?_0\) встречается как в левой так и в правой части уравнения, значит этот терм не типизируется
    \end{proof}
...
\end{minted}
    ...
    \begin{proof}
        $\lambda f~x\ldotp f \ap x \eqeta \lambda f\ldotp f$
    \end{proof}
