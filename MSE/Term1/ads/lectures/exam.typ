...
Очев.
== #todo() MergeSort. _Подсчёт числа инверсий в массиве._
...
== #todo() _Двоичный поиск: сколько раз число встречается на отрезке? за $cal(O)(log n)$._
== #todo() _Двоичный поиск по ответу: отправка коров в стойла так, чтобы $min "dist" -> max$._
...
== #todo() _Нижняя оценка на время сортировки, корректной для $1 / 100^n $ доли перестановок: $Omega(n log n)$._
...
== #todo() Понятие вероятностного алгоритма. Отличие понятий "амортизированно" и "в среднем".
...
== #todo() QuickSort. Доказательство заменой суммы на интеграл.
...
== #todo() _$k$-ая статистика: детерминированная с делением на кусочки длины 3 и 7._
...
== #todo() _Структура для $n$ чисел от $1$ до $k$ за $chevron.l cal(O)(n + k), cal(O)(1) chevron.r$ говорящая "сколько чисел от a до b"?_
...
- Динамика вперед --- знаем ответ для текущего состояния, обновляем (релаксируем) значение для "следующих" состяония
...
Динамика назад:
#pseudocode-list()[
  + *for* $v <- t - 1, dots, s$ --- в порядке топсортв порядке топсорта
...
Будем хранить в $"dp"_i$ дополнительно последний элемент этой последовательности. Понятно (?) что у наибольшей последовательности на префиксе последний элемент будет не меньше чем у наибольшей последовательности на предыдущем префиксе. Получается что имеем на префиксе "отсортированный" массив $a_(p_j)$. Заметим что чтобы получить новую наибольшую последовательность для очередного префикса с последним элементов $a_i$ нужно найти последовательность с максимальной длиной у которой последний элементп $> a_i$. Можем найти такой $a_j$ бинпоиском. Затем взять максимальную последовательность из найденой с элементом $a_i$ и из максимальной последовательности без $a_i$. Т.е.
$ chevron.l a_j, l chevron.r = max_l cases("upper_bound"("dp"_(0 dots i - 1), a_i) + 1, "dp"_(i - 1)) $
...
- $"dp"_(i, c)$ --- минимальный вес $sum_(i in I) c_i = c$.
- $"dp"_(i, c) = min cases("dp"_(i - 1, c), w_i + "dp"_(i - 1, c - c_i))$
- Ответ: $max c : "dp"_(n, c) <= W$
...
== #todo() _Рюкзак с линией памяти. Версии рюкзака, когда предмет можно брать $1$ или $+infinity$ раз._
== Рюкзак с bitset.
...
#section[Динамика-2]
== #theormin #todo() DP по дереву: размер поддерева, глубина.
...
== #todo() _Операции с множествами за $cal(O)(1)$: найти младший единичный бит._
...
== #todo() _Задача SetCover: покрыть множество B минимальным числом $A_i$. Решение за $O(2^(|B|) m)$._
== #todo() Пребор пар "множество, подмножество" за $3^n$.
...
Как сделать check --- проверять что из одной маски можно получить другую на "следующем уровне".
#pseudocode-list()[
  + *if* $m_1 union m_2 != emptyset$
    + *return* _false_
  + $m <- m_1 union m_2$
  + *return* $d_m$
]
...
== #todo() Хаффман. Хранение дерева/частот.
...
Очев.
== #todo() dfs: поиск пути, обратный ход рекурсии.
...
== #todo() dfs: поиск цикла в орграфе.
== #todo() _dfs: поиск цикла в неорграфе._
== #todo() _dfs: покраска в два цвета._
...
== #todo() _2-связность: поиск мостов и компонент (dp по дереву)._
== 2-SAT.
#definition()[
...
== #todo() BFS. _Модификация для целых $1-k$ весов за $O(E + k V)$ и $O(E log k)$._
== #todo() BFS. _Модификация для вещественных $1-k$ весов._
== #todo() BFS. _Модификация для целых $0-1$ весов._
== Дейкстра. Алгоритм и доказательство корректности.
#proof()[
  Пусть есть вершины для которых уже знаем расстояние $A$ и все остальные для которых есть кандидат расстояние $V \\ A$. Если взять $v -> u: v in A$ и $u in V \\ A$, есть потонциальное расстояние до $u$ $d_u + w_(v u)$. Покажем что это $d_u + w(v u)$ это минимальное расстояние до $u$.
...
== #todo() Реализация на C++/python: `set`, `prioirity_queue`, `operator<`, `heapq.heappush`.
