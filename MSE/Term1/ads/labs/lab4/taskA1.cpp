...
// Generated at 2025-06-10 09:25:44.619081 
// By iliayar
...
bool operator<(const int& n, const S& s) {
    return n < s.v;
}
bool operator<(const S& s, const int& n) {
    return s.v < n;
}
...
void sol() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (cmd.rfind("Insert", 0) == 0) {
            int k = atoll(string_view(cmd).substr(7, -1).data());
