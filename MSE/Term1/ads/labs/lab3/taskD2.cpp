...
// Generated at 2024-09-29 00:02:33.554866 
// By iliayar
...
void sol() {
  ...
  for (int i = 0; i < k; ++i) {
    ...
    l--;
    r--;
    auto [_1, fl] = bs(0, a.size(), [&](int m) { return a[m].first < x; });

    if (fl >= a.size() || a[fl].first != x) {
        cout << "0";
        continue;
    }

    auto [_2, fr] = bs(0, a.size(), [&](int m) { return a[m].first <= x; });

    // DBG() << fl << " " << fr << endl;

    auto [fil, _3] = bs(fl, fr, [&](int m) { return a[m].second < l; });
    auto [_4, fir] = bs(fl, fr, [&](int m) { return a[m].second <= r; });
